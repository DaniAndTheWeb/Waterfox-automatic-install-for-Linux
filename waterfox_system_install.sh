...
#
# This file installs Waterfox (beta release).

## SUSE Patch -- Will safely skip if not using a SUSE based distribution and continue with normal install of Waterfox. ##
...
display_error_message() {
  printf -- '\n%s\n' "" "" "" "Oops, something went wrong!" \
    "I apologize, but I couldn't download Waterfox." \
...
# Download notice.
printf '\n\n%s\n\n' "Please wait. I am downloading the latest version of Waterfox."
...
printf "Do you want to specify a custom installation path? (Default is /opt/Waterfox) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Waterfox): "
    read -r install_path
...
# Required permissions needed for Waterfox automatic update feature to work.
sudo chmod -R 757 "/$install_path/" ;

# Create icon script.
printf "[Desktop Entry]
Exec=$install_path/waterfox %%u --class Waterfox
...
Exec=$install_path/waterfox --new-window %%u --class Waterfox
Name=New Window
...
Exec=$install_path/waterfox --private-window %%u --class Waterfox
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Waterfox.desktop ;

# Makes the icon executable, allowing it to run Waterfox.
chmod +x Waterfox.desktop
...
sudo cp Waterfox.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Waterfox.desktop '{}' \; -exec chown --reference='{}' '{}/Waterfox.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Waterfox.desktop /etc/skel/Desktop
...
# Exit notice.
printf -- '%s\n' "" "" "🎉 Congratulations! 🎉" \
    "Waterfox has been successfully installed on your computer." \
    "From now on, Waterfox will handle its own updates automatically." \
